#######################################################################################################################################################
# $Id: 98_MAX_Temperatur.pm 14.05.2019 21:00
# 
...
#####################################
sub MAX_Temperatur_Initialize($)
{
	my ($hash) = @_;
	$hash->{DefFn}		= "MAX_Temperatur_Define";
	$hash->{UndefFn}	= "MAX_Temperatur_Undef";
	$hash->{AttrFn}		= "MAX_Temperatur_Attr";  
	$hash->{SetFn}		= "MAX_Temperatur_Set";
	#$hash->{GetFn}		= "MAX_Temperatur_Get";		#Derzeit kein Get-Befehl
	#$hash->{ParseFn}	= "MAX_Temperatur_Parse";
	...
	$hash->{NotifyFn}	= "MAX_Temperatur_Notify";
	$hash->{FW_summaryFn}	= "MAX_Temperatur_summaryFn";          	# displays html instead of status icon in fhemweb room-view
	
	#$hash->{FW_hideDisplayName} = 1;                     			# Forum 88667 
	#$hash->{FW_detailFn}	= "MAX_Temperatur_summaryFn";         	# displays html instead of status icon in fhemweb room-view
	#$data{webCmdFn}{MAX_Temperatur} = "MAX_Temperatur_webCmdFn";	# displays rc instead of device-commands on the calling device
	#$hash->{FW_atPageEnd} = 1; 									# wenn 1 -> kein Longpoll ohne informid in HTML-Tag
}

###################################
sub MAX_Temperatur_Define($$){
	my ($hash, $def) = @_;
	my $name = $hash->{NAME};
	my @a = split("[ \t][ \t]*", $def);
	...
	return "Wrong syntax: use define <name> MAX_Temperatur <Optional:T|HT|WT|addr>" if (!$ok);
...
#####################################
sub MAX_Temperatur_Undef($$) {
	my ($hash, $name) = @_;
	delete($modules{MAX_Temperatur}{defptr}{$hash->{DEF}}) if(defined($hash->{DEF}) && defined($modules{MAX_Temperatur}{defptr}{$hash->{DEF}}));
	return undef;
}

#####################################
sub MAX_Temperatur_Attr(@) {
...
sub MAX_Temperatur_Set($@){
	...
	if ($cmd eq "reset"){
		...
		MAX_Temperatur_SetDevice($hash, $val);
		#MAX_Temperatur_Reset($hash);
		return "ok";
	}
	if ($cmd eq "start"){
		my $retValue = MAX_Temperatur_Execute($hash);
...
sub MAX_Temperatur_SetDevice($$){
	...
	my $MaxDevice = MAX_Temperatur_GetDeviceFromName($hash,$device);
...
sub MAX_Temperatur_Notify($$){
	...
	#Wenn Structure
	if ($IsMyDevice == 0){
		my @StrucDevices = MAX_Temperatur_GetStructureDevices($hash, ReadingsVal($name,"Selected_MaxDevice",""));
	...
	my $state = Max_Temperatur_GetDevState($hash);
	return undef;
}

#####################################
sub MAX_Temperatur_summaryFn($$$$){
...
sub MAX_Temperatur_HTML($){
	...
	my $htmlState = Max_Temperatur_GetDevState($hash);
	
	my $htmlDev = MAX_Temperatur_GetHTMLSelectMaxDevice($hash);
	my $htmlMode = MAX_Temperatur_GetHTMLSelectMode($hash);
	my $htmlTemp = MAX_Temperatur_GetHTMLSelectTemperaturen($hash);
	my $htmlDatum = MAX_Temperatur_GetHTMLSelectDatum($hash);
	my $htmlUhrzeit = MAX_Temperatur_GetHTMLSelectUhrzeit($hash);
...
sub MAX_Temperatur_GetStructureDevices($$){
	my $hash = shift;
	my $name = $hash->{NAME};
...
sub Max_Temperatur_GetDevState($){
	my $hash = shift;
	my $name = $hash->{NAME};
	...
	#Prüfen, ob es eine structure ist:
	my @StrucDevices = MAX_Temperatur_GetStructureDevices($hash, $maxdev);
	if (scalar @StrucDevices == 0){
...
sub MAX_Temperatur_GetHTMLSelectMaxDevice($){
	my $hash = shift;
	my $name = $hash->{NAME};
	
	my @Devices=MAX_Temperatur_GetDevices($hash);
	...
	if (scalar @Devices > 1){
		...
		if (scalar @Devices eq 1){
			...
			if (ReadingsVal($name, "Selected_Device", "---") eq "---"){
				MAX_Temperatur_SetDevice($hash, $Devices[0]);
			}
		} else {
			$html.="not defined";
		}
...
sub MAX_Temperatur_GetDeviceFromName($$){
	my $hash = shift;
	my $caption = shift;
	my $name = $hash->{NAME};
...
#####################################
#HTML Select Mode
sub MAX_Temperatur_GetHTMLSelectMode($){
	my $hash = shift;
	my $name = $hash->{NAME};
	
	#Log3 $name,1,"$name, event MAX_Temperatur_GetHTMLSelectMode";
	
	my $mode	= "---,auto,manual";
	my $html	= MAX_Temperatur_GetHTMLSelectListe($hash, $mode,"Selected_Mode");
...
sub MAX_Temperatur_GetHTMLSelectTemperaturen($){
	my $hash = shift;
	my $name = $hash->{NAME};
	...
	my $html	= MAX_Temperatur_GetHTMLSelectListe($hash, $temp,"Selected_Temperatur");
	return $html;
}

#####################################
#HTML Select Datum
sub MAX_Temperatur_GetHTMLSelectDatum($){
	my $hash = shift;
	my $name = $hash->{NAME};
	...
	my $html	= MAX_Temperatur_GetHTMLSelectListe($hash, $d,"Selected_Datum");
	return $html;
...
#####################################
#HTML Select Uhrzeit
sub MAX_Temperatur_GetHTMLSelectUhrzeit($){
	my $hash = shift;
	my $name = $hash->{NAME};
  ...
  for (my $i=0;$i<$maxHour*2;$i++)
	...
	my $html	= MAX_Temperatur_GetHTMLSelectListe($hash, $t,"Selected_Uhrzeit");
	return $html;
}

#####################################
#HTML Select nach Liste
sub MAX_Temperatur_GetHTMLSelectListe($$$){
	my $hash = shift;
	my $Names = shift;
...
sub MAX_Temperatur_GetHTMLSend($){
...
sub MAX_Temperatur_GetHTMLReset($){
	#entweder Button oder Text
	my $hash = shift;
	my $name = $hash->{NAME};
